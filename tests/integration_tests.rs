...
use anyhow::Result;
use js_cleanifier::{JSCleanifier, CleanifyOptions};
...
async fn test_simple_js_file() -> Result<()> {
    ...
    cleanifier.cleanify_file_to_output(&testfile_path, &options).await?;
...
async fn test_large_js_file() -> Result<()> {
    ...
    let result = cleanifier.cleanify_file_to_output(&target_path, &options).await;
...
async fn test_verbose_flag() -> Result<()> {
    ...
    // Test with verbose flag (though it currently doesn't affect library behavior)
    ...
    assert!(result.contains("function"), "Result should contain function");
...
async fn test_invalid_output_path() -> Result<()> {
    ...
    let result = cleanifier.cleanify_file_to_output(&testfile_path, &options).await;
